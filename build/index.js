...
(function () {
	...
	var SteamTIFF = function () {
		...
		this.init = () =>
			new Promise((resolve, reject) => {
				...
				try {
					...
					this.begin().then(this.webServer.begin).then(this.socketServer.begin).then(this.serialPort.begin).then(this.initialize).then(resolve);
		...
		this.loadModules = this.loadModule = (moduleName) =>
			new Promise((resolve, reject) => {
					...
					if (message) {
						...
						if (message.broadcast === true) {
							...
							if (this.socketServer.io) {
								this.socketServer.io.emit(message.evt, message.data);
							}
							if (message.evt.indexOf('.') !== -1) {
								this.emit(message.evt.split('.')[0], message.data);
								if (this.socketServer.io) {
									this.socketServer.io.emit(message.evt.split('.')[0], message.data);
								}
							}
							this.emit('*', message);
						}
	...
	SteamTIFF.init().then(SteamTIFF.ready);
})();
